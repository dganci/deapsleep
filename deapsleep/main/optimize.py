...
def run_experiment(params):
    ...
    problem, n_runs, n_var, save_results, aggr_op, version, seed = d_.extract_params(
        params, 
    ...
    if params.get('use_opt_config', False):
        cfg_path = os.path.join(CONFIG_DIR, problem, f"{problem}_best_config.pkl")
    ...
    # bbox formatting
    if 'instance' in params and 'bbob' in problem:
        problem += f"-{params['instance']}"
