    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        if self.use_dropout:
            ...
            try:
                ...
                if self.strg == 'remove': # variables removal
                    sub_x = x[:, mask]
                    self.problem.n_var = int(mask.sum())

                elif self.strg == 'substitute': # variables substitution
                ...
                elif self.strg == 'bit_rearrange': # random bits rearrangement
                    sub_x = np.asarray(x.copy()).reshape(-1)
                    for i in np.where(mask)[0]:
                        sub_x[i] = random.choice([0, 1])
                    sub_x = sub_x.reshape(1, -1)
                
                elif 'permute' in self.strg:  # permute active elements
                    sub_x = np.asarray(x.copy()).reshape(-1)
                    if self.strg == 'permute_tsp':
                        idxs = np.where(
                            mask & (np.arange(len(mask)) != 0)
                        )[0] # first element (0) ix excluded from permutation for tsp-like problems
                        if len(idxs) < 2: # at least 2 positions to permute
                            idxs = np.arange(1, self.n_var) \
                            if self.n_var > 1 \
                            else np.arange(self.n_var)
                    else:
                    ...
                    raise ValueError(
                        ...
                        'Please specify "removal", "substitution", ' \
                        '"bit_rearrange", "permute" or "permute_tsp".'
                    )       
                
                # Evaluate the modified individual
                self.problem._evaluate(sub_x, out, *args, **kwargs)

                #if self.strg not in ('bit_rearrange', 'permute'):
                # Save the mask for partial variation
                out['mask'] = mask.astype(int).tolist()
                # Apply the correction factor to the fitness
                corr = 1 / (1 - self.rate)
                out["F"] = out["F"] * corr
