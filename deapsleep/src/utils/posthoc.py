...
class PostHocAnalysis:
        ...
        for version, data in self.hofs.items():
            for run_idx, solutions in enumerate(data['best'], start=1):
                for sol_idx, indiv in enumerate(solutions, start=1):
                    fit = np.atleast_1d(indiv.fitness.values).flatten()
                    record = {
                        'Run': run_idx,
                        'ID': sol_idx,
                        'Version': version,
    ...
    def plot_scatterplot(self, d1, d2, path: str = None, filename: str = None):
        ...
        g = sns.FacetGrid(df, col="Metric", hue="Version", sharey=False,
                        col_wrap=2, height=4, palette=self.colors)
    ...
    def plot_boxplots(self, path: str, filename: str, obj: int = 0):
            ...
            for i, run in enumerate(runs):
                for j, version in enumerate([self.v1, self.v2]):
                    ...
                    ax.bxp([stats],
                        ...
                        widths=box_width,
                        patch_artist=True,
                        boxprops=dict(facecolor=self.colors.get(version), alpha=0.7),
                        showfliers=False,
                        showmeans=True,
                        meanline=True,
        ...
        handles = [
            Patch(facecolor=self.colors[self.v1], alpha=0.7, label=self.v1),
            Patch(facecolor=self.colors[self.v2], alpha=0.7, label=self.v2),
